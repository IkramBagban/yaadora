...
/** Second confirmation: type-to-confirm. Owns its phrase state and is mounted
 * fresh when the dialog advances, so no effect-based reset is needed. */
function TypeConfirmStep({
  busy,
  onClose,
  onConfirm,
}: {
  ...
  const ready = phrase === CONFIRM_PHRASE && !busy;

  return (
    <>
      <p className="mt-md text-sub text-ink2">
        Final step — type <strong>{CONFIRM_PHRASE}</strong> to start the rebuild.
      </p>
      <input
        autoFocus
        value={phrase}
      ...
      <DialogActions busy={busy} onClose={onClose}>
        <Button variant="secondary" className={dangerButton} onClick={onConfirm} disabled={!ready}>
...
function WarningStep({
  ...
  return (
    <>
      <p className="mt-md text-sub text-ink2">
        Facts, entities, relationships, open loops and digests will be rebuilt
        from your raw memories. Raw memories are immutable and are never
        modified. This can take a while.
      </p>
      <DialogActions busy={false} onClose={onClose}>
        <Button variant="secondary" className={dangerButton} onClick={onConfirm}>
          Continue
        </Button>
      </DialogActions>
    </>
...
function ConfirmDialog({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Confirm derived-state rebuild"
    >
      <button
        type="button"
      ...
      <div className="relative w-full max-w-md rounded-lg border border-hairline bg-surface p-xl shadow-xl">
        ...
        {step === 'warning' ? (
          <WarningStep onClose={onClose} onConfirm={onConfirm} />
        ) : (
          <TypeConfirmStep busy={busy} onClose={onClose} onConfirm={onConfirm} />
        )}
      </div>
    </div>
  );
