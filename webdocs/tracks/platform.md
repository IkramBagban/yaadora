...
- `request<T>` blindly casts parsed JSON — wire-shape mismatches are runtime-only errors → list endpoints use envelopes (`GET /entities` → `{ entities }`, not a bare array); always type the envelope at the fetcher → 2026-08-22
